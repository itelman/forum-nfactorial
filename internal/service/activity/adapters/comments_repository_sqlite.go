...
func (r *CommentsRepositorySqlite) GetAllForPostByUser(input domain.GetAllCommentsForPostByUserInput) ([]*dto.Comment, error) {
	...
	rows, err := stmt.Query(input.AuthUserID, input.PostID, input.AuthUserID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
